...
module.exports.run = async function ({ api, event, args }) {
	...
	if (!args[0]) return out("☘️🌺জারে এড দিবেন তার লিংক বা uid দিয়ে ট্যাই করুন🌺☘️");
	if (!isNaN(args[0])) return adduser(args[0], undefined);
	else {
		try {
			var [id, name, fail] = await getUID(args[0], api);
			if (fail == true && id != null) return out(id);
			else if (fail == true && id == null) return out("☘️🌺নলা মার্কা আইডি এড দেইনা গ্রুপে🌺uid দিয়া ট্যাই করেন🌺☘️")
			else {
	...
	async function adduser(id, name) {
		id = parseInt(id);
		if (participantIDs.includes(id)) return out(`${name ? name : "এই মেম্বার"} গ্রুপেই আছে কানার নাতী🦋.`);
		else {
			...
			if (approvalMode === true && !admins.includes(botID)) return out(`🍁নতুন মেম্বার🌿 ${name ? name : "এড ডান"} ☘️🌺এড তো দিলাম Apprve করবে কেডা☘️🌺`);
			else return out(`🌺এড ডান ${name ? name : "member"} গ্রুপে☘️`)
		}
	}
                   }